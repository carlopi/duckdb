...
unique_ptr<FileBuffer> StandardBufferManager::ReadTemporaryBuffer(QueryContext context, MemoryTag tag,
	...
	if (offset + GetAllocSize(block_header_size + block_size) != file_size) {
		throw DataCorruptionException("Corrupt temporary file \"%s\": header describes %llu bytes but file is %llu",
		                              path, offset + GetAllocSize(block_header_size + block_size), file_size);
	}
