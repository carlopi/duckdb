...
unique_ptr<FileBuffer> TemporaryFileHandle::ReadTemporaryBuffer(QueryContext context,
	...
	if (compressed_size > compressed_buffer.GetSize() - sizeof(idx_t)) {
		throw DataCorruptionException(
		    "Corrupt temporary file: compressed block claims %llu bytes but only %llu are available", compressed_size,
	...
	if (duckdb_zstd::ZSTD_isError(decompressed_size)) {
		throw DataCorruptionException("Corrupt temporary file: failed to decompress block (%s)",
		                              duckdb_zstd::ZSTD_getErrorName(decompressed_size));
	}
	if (decompressed_size != buffer->AllocSize()) {
		throw DataCorruptionException("Corrupt temporary file: decompressed block is %llu bytes but expected %llu",
