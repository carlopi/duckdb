...
string ExtensionHelper::ExtensionInstalDocumentationLink(const string &extension_name) {
...
static unique_ptr<ExtensionInstallInfo> InstallFromHttpUrl(DatabaseInstance &db, const string &url,
	...
	while (true) {
		...
		if (!should_retry || retry_count >= MAX_RETRY_COUNT) {
			...
			auto documentation_link = ExtensionHelper::ExtensionInstalDocumentationLink(extension_name);
