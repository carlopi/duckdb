...
public:
	typename std::add_lvalue_reference<typename get_inner<_Tp>::inner>::type operator*() const {
	...
	typename std::add_lvalue_reference<typename get_inner<_Tp>::inner>::type operator[](size_t __i) const {
		const auto ptr = original::get();
		if (MemorySafety<SAFE>::enabled) {
			AssertNotNull(!ptr);
		}
