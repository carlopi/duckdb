...
//! ADOPT registers an already-provisioned resource from a handle value; DESTROY tears one down and
//! deregisters it; SHOW lists the registered resources.
enum class ExternalResourceOperation : uint8_t { CREATE, ADOPT, DESTROY, SHOW };
...
public:
	...
	//! The resource type ('<recipe>') expression. Set for CREATE and ADOPT.
	unique_ptr<ParsedExpression> type;
	//! The local name: the `AS <name>` alias for CREATE/ADOPT (optional), or the target for DESTROY.
	Identifier name;
	//! Create params `(k v, ...)`. Set for CREATE.
	case_insensitive_map_t<unique_ptr<ParsedExpression>> options;
	//! The `FROM <value>` handle expression. Set for ADOPT.
