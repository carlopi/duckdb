...
const char INLINED_PEG_GRAMMAR[] = {
    ...
    "TypeVariations <- TimeType / IntervalType / BitType / RowType / VariantType / MapType / TupleType / GeometryType "
    "/ UnionType / NumericType / SetofType / SimpleType\n"
    ...
    "        RenameColumn / RenameAlter / SetPartitionedBy / ResetPartitionedBy / SetSortedBy / ResetSortedBy / "
    ...
    "CopyFileName <- CopyFileNameExpression / CopyFileNameStringLiteral / CopyFileNameIdentifier / "
    ...
    "CopySchema <- 'SCHEMA'\n"
    "CopyData <- 'DATA'\n"
    "CreateIndexStmt <- UniqueIndex? 'INDEX' IfNotExists? IndexName? 'ON' BaseTableName InsertColumnList? IndexType? "
    "Parens(List(IndexElement))? WithList? WhereClause?\n"
    ...
    "CreateStatementVariation <- CreateTableStmt / CreateMacroStmt / CreateSequenceStmt / CreateTypeStmt / "
    "CreateSchemaStmt / CreateViewStmt / CreateIndexStmt / CreateSecretStmt / CreateTriggerStmt\n"
    ...
    "QualifiedName <- CatalogReservedSchemaIdentifier / SchemaReservedIdentifierOrStringLiteral / "
    "IdentifierOrStringLiteral\n"
    "SchemaReservedIdentifierOrStringLiteral <- SchemaQualification ReservedIdentifierOrStringLiteral\n"
    "CatalogReservedSchemaIdentifier <- CatalogQualification ReservedSchemaQualification "
    ...
    "/ ForeignKeyConstraint / ColumnCollation / ColumnCompression\n"
    ...
    "TopLevelConstraintList <- TopPrimaryKeyConstraint / TopCheckConstraint / TopUniqueConstraint / "
    ...
    "ColLabel <- ReservedKeyword / UnreservedKeyword / ColumnNameKeyword / FuncNameKeyword / TypeNameKeyword / "
    "Identifier\n"
    ...
    "ReferencingClause? ForEachClause? TriggerBody\n"
    ...
    "CreateViewStmt <- CreateRecursive? 'VIEW' IfNotExists? QualifiedName InsertColumnList? WithList? 'AS' "
    ...
    "ReservedTableQualification ReservedColumnName\n"
    ...
    "FunctionExpression <- FunctionIdentifier FunctionExpressionArguments WithinGroupClause? FilterClause? "
    "ExportClause? OverClause?\n"
    ...
    "FunctionIdentifier <- CatalogReservedSchemaFunctionName / SchemaReservedFunctionName / "
    ...
    "LikeVariations <- SimilarToToken / RegexInsensitiveMatchToken / RegexMatchToken / ILikeToken / LikeToken / "
    "GlobToken / NotILikeOp / NotLikeOp / NotRegexInsensitiveMatchOp / NotSimilarToOp\n"
    "LikeToken      <- 'LIKE' / '~~'\n"
    "ILikeToken     <- 'ILIKE' / '~~*'\n"
    "GlobToken      <- 'GLOB' / '~~~'\n"
    ...
    "PostfixOperator <- '!'\n"
    "SpecialFunctionExpression <- CoalesceExpression / UnpackExpression / TryExpression / ColumnsExpression / "
    "ExtractExpression / LambdaExpression / NullIfExpression / PositionExpression / RowExpression / "
    "SubstringExpression / TrimExpression / OverlayExpression\n"
    ...
    "ExternalResourceStatement <- CreateExternalResourceStmt / AdoptExternalResourceStmt / DestroyExternalResourceStmt "
    "/ ShowExternalResourcesStmt\n"
    "CreateExternalResourceStmt <- 'CREATE' 'EXTERNAL' 'RESOURCE' Expression AttachAlias? AttachOptions?\n"
    "AdoptExternalResourceStmt <- 'ADOPT' 'EXTERNAL' 'RESOURCE' Expression AttachAlias? 'FROM' Expression\n"
    ...
    "ShowAllModifier <- 'ALL'\n"
    "InsertStatement <- WithClause? 'INSERT' OrAction? 'INTO' InsertTarget ByNameOrPosition? InsertColumnList? "
    "InsertValues OnConflictClause? ReturningClause?\n"
    ...
    "MergeIntoStatement <- WithClause? 'MERGE' 'INTO' TargetOptAlias MergeIntoUsingClause JoinQualifier MergeMatch+ "
    "ReturningClause?\n"
    ...
    "MatchedClauseAction <- UpdateMatchClause / DeleteMatchClause / InsertMatchClause / DoNothingMatchClause / "
    "ErrorMatchClause\n"
    ...
    "SetopExcept <- 'EXCEPT'\n"
    "ByName <- 'BY' 'NAME'\n"
    "SelectStatementType <- OptionalParensSimpleSelect / ValuesClause / DescribeStatement / TableStatement / "
    "PivotStatement / UnpivotStatement\n"
    ...
    "ZoneValue <- ZoneIntervalWithPrecision / ZoneIntervalWithInterval / ZoneLocal / ZoneDefault / ZoneStringLiteral / "
    "ZoneIdentifier / NumberLiteral\n"
    "ZoneLocal <- 'LOCAL'\n"
    "ZoneDefault <- 'DEFAULT'\n"
    ...
    "WithExternalResourceAttach <- 'WITH' 'EXTERNAL' 'RESOURCE' Expression AttachAlias? AttachOptions? 'ATTACH' "
    "AttachAlias? AttachOptions?\n"
    "WithExternalResourceConnect <- 'WITH' 'EXTERNAL' 'RESOURCE' Expression AttachAlias? AttachOptions? 'CONNECT' "
    "AttachOptions?\n"
