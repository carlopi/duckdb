...
//! ATTACH/CONNECT and WITH EXTERNAL RESOURCE option handling.
...
unique_ptr<SQLStatement> PEGTransformerFactory::TransformCreateExternalResourceStmt(
	...
	result->type = std::move(expression);
	if (attach_alias) {
...
unique_ptr<SQLStatement> PEGTransformerFactory::TransformAdoptExternalResourceStmt(
    PEGTransformer &transformer, unique_ptr<ParsedExpression> expression, const optional<Identifier> &attach_alias,
    unique_ptr<ParsedExpression> expression_1) {
	auto result = make_uniq<ExternalResourceStatement>(ExternalResourceOperation::ADOPT);
	result->type = std::move(expression);
	if (attach_alias) {
		result->name = Identifier(*attach_alias);
	}
	result->handle = std::move(expression_1);
	return std::move(result);
}
