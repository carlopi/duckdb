...
static void FillFunctionParameters2(FunctionDescription &function_description, const char *function_name,
...
template <class T>
void FillFunctionDescriptions2(const StaticFunctionDefinition &function, T &info) {
	...
	for (idx_t variant_index = 0; variant_index < variants.size(); variant_index++) {
		...
		FillFunctionParameters2(function_description, function.name, parameters, descriptions, examples);
		// description
...
static void FillExtraInfo(const StaticFunctionDefinition &function, T &info) {
	info.internal = true;
	FillFunctionDescriptions2(function, info);
